...
public class ServerImpl implements ServerInterface {
    ...
    public void sendSlotDataPacket(Map<String, String> map) {
        PacketDistributor.sendToAllPlayers(new SyncSlotDataPacket(map));
    }

    @Override
    public void sendCheckPacket(String s) {
        PacketDistributor.sendToAllPlayers(new GetCheckPacket(s));
    }

    @Override
    public void sendChecksDataPacket(List<String> list) {
        PacketDistributor.sendToAllPlayers(new StartSyncChecksPacket(list));
    }
    ...
    public AdvancementInterface getAdvancement(String s) {
        return new AdvancementImpl(this.server.getAdvancements().tree().get(ResourceLocation.parse(s)));
    }
