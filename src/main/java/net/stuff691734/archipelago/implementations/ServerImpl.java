...
public class ServerImpl implements ServerInterface {
    ...
    public void sendSlotDataPacket(Map<String, String> map) {
        this.server.getPlayerList().getPlayers().forEach(
            (player) -> PacketDistributor.PLAYER.with(player).send(new SyncSlotDataPacket(map))
        );
    }
    ...
    public void sendCheckPacket(String s) {
        this.server.getPlayerList().getPlayers().forEach(
            (player) -> PacketDistributor.PLAYER.with(player).send(new GetCheckPacket(s))
        );
    }
    ...
    public void sendChecksDataPacket(List<String> list) {
        this.server.getPlayerList().getPlayers().forEach(
            (player) -> PacketDistributor.PLAYER.with(player).send(new StartSyncChecksPacket(list.toArray(String[]::new)))
        );
    }
