...
public class EntityLoadEvent {
    ...
    public void onEvent(PlayerEvent.PlayerLoggedInEvent event) {
        ...
        if (server != null) {
            ...
            int playerLastCheck = state.playerLastCheck.getOrDefault(event.player.getCachedUniqueIdString(), 0);
            if (serverLastCheck > playerLastCheck) {
                state.playerLastCheck.put(event.player.getCachedUniqueIdString(), serverLastCheck);
