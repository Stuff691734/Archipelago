...
public class EntityLoadEvent {
    ...
    public void onEvent(PlayerEvent.PlayerLoggedInEvent event) {
        ...
        if (server != null) {
            ...
            PacketDistributor.PLAYER.with((ServerPlayer) event.getEntity()).send(new StartSyncChecksPacket(state.checks.keySet().toArray(new String[0])));
            if (!state.slotData.isEmpty()) {
                PacketDistributor.PLAYER.with((ServerPlayer) event.getEntity()).send(new SyncSlotDataPacket(state.slotData));
            }
        }
    }
}
