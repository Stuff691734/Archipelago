...
public class GetCommand {
    public static int execute(CommandContext<CommandSource> context) {
        if (ArchipelagoPersistentState.getInstance() != null) {
            context.getSource().sendFeedback(new StringTextComponent(ArchipelagoPersistentState.getInstance().checks.toString()), true);
    ...
    public static int executeSpecific(CommandContext<CommandSource> context) {
        ...
        if (ArchipelagoPersistentState.getInstance() != null) {
            context.getSource().sendFeedback(new StringTextComponent(String.valueOf(ArchipelagoPersistentState.getCheck(check[1] + " " + check[2]))), true);
        }
        return 0;
    }
}
